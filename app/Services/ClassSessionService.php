...
final class ClassSessionService
{
    ...
    public function getAllClassSessions(User $user, array $filters = [])
    {
        if ($user->role === "admin-pegawai" || $user->role === "super-admin") {
            ...
            return $this->repository->getAllByLecturer(
                $user->detail_id,
                $filters,
            );
        } else {
    ...
    {
        return $this->repository->getAssignmentByCourse($attributes);
    }
    ...
    public function addSessionMaterial(
        ClassSession $classSession,
        array $attributes,
    ) {
        return DB::transaction(function () use ($classSession, $attributes) {
            return $this->repository->createSessionMaterial(
                $classSession,
                $attributes,
            );
        });
    ...
    public function deleteSessionMaterial(
        ClassSession $classSession,
        array $attributes,
    ) {
        return DB::transaction(function () use ($classSession, $attributes) {
            return $this->repository->deleteSessionMaterial(
                $classSession,
                $attributes,
            );
        });
